...
-- WILL IMPLEMENT A CENTRALISED RESERVATION STATION SYSTEM
...
entity decode is
	...
	port(clk	: in std_logic;
		  ...
		  REG1	: out std_logic_vector(60 downto 0);
		  REG2	: out std_logic_vector(60 downto 0));
...
begin
	...
	out_reg1 : registers generic map(N => 61) port map(clk => clk,
																	   reset => reset,
																	   input => reg1_in,
																	   output => REG1,
																	   enable => not_stall);
	
	out_reg2 : registers generic map(N => 61) port map(clk => clk,
																	   reset => reset,
																	   input => reg2_in,
																	   output => REG2,
																	   enable => not_stall);
