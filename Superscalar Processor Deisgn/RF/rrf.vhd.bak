...
entity RRF is
	...
	port(reset 	 		: in std_logic;
		  clk   	 		: in std_logic;
		  in_sel1 		: in std_logic_vector(natural(log2(real(N)))-1 downto 0);
		  in_sel2		: in std_logic_vector(natural(log2(real(N)))-1 downto 0);
		  ...
		  osel1	 		: in  std_logic_vector(natural(log2(real(N)))-1 downto 0);
		  osel2	 		: in  std_logic_vector(natural(log2(real(N)))-1 downto 0));
...
begin
	...
	mux1 : multiplexer generic map(X => N, Y => 16) 
							 port map(output => output1,
										 input  => data_out,
										 sel	  => osel1);
	
	mux2 : multiplexer generic map(X => N, Y => 16) 
	...
	process(in_sel1,in_sel2,input1,input2,wren1,wren2)
		variable inp : main_array(0 to N-1)(15 downto 0) := ((others => (others => '0')));
		begin
